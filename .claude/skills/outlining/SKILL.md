---
name: outlining
description: Story structure and chapter-by-chapter outline creation
allowed-tools:
  - get_concept
  - get_brief
...
1. **Start** by calling `get_concept` to retrieve the expanded book concept
2. **Reference** the original intent by calling `get_brief`
3. **Enrich** with existing character data by calling `get_character_profiles`
4. **Ground** in setting details by calling `get_world_building`
