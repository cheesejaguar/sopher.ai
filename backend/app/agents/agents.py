...
class BookWritingAgents:
    ...
    async def generate_concepts(
        self, brief: str, plot_seeds: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        crew = Crew(agents=[self.concept_generator], tasks=[task], verbose=False)
    ...
    async def create_outline(
        ...
        crew = Crew(agents=[self.outliner], tasks=[task], verbose=False)
    ...
    async def write_chapter(
        ...
        crew = Crew(agents=[self.writer], tasks=[task], verbose=False)

        result = await asyncio.to_thread(crew.kickoff)
        return str(result)

    async def edit_content(
        self, content: str, edit_type: str = "structural", instructions: Optional[str] = None
    ) -> str:
        ...
        crew = Crew(agents=[self.editor], tasks=[task], verbose=False)
    ...
    async def check_continuity(
        ...
        crew = Crew(agents=[self.continuity_checker], tasks=[task], verbose=False)

        result = await asyncio.to_thread(crew.kickoff)

        try:
            # Try to parse as JSON, fallback to structured dict
            report: Dict[str, Any] = json.loads(str(result))
