...
export function Hero() {
  return (
    <section className="marketing-canvas marketing-hero-stage relative overflow-hidden border-b border-black/10 dark:border-white/10">
      ...
      />
      <div className="relative z-10 mx-auto grid min-h-[calc(100svh-4rem)] w-full max-w-7xl items-start gap-3 px-4 py-7 sm:gap-4 sm:px-6 sm:py-10 lg:grid-cols-12 lg:items-center lg:gap-10 lg:px-8 lg:py-20">
        {/*
        ...
        */}
        <div className="contents lg:col-span-6 lg:block lg:pr-8">
          <h1 className="order-1 min-w-0 max-w-full font-display text-[clamp(2.35rem,12vw,5.6rem)] font-semibold leading-[0.94] tracking-[-0.04em] text-balance sm:max-w-3xl sm:text-[clamp(2.8rem,6.4vw,5.6rem)] sm:leading-[0.92]">
            The book in your head,{" "}
            <span className="mt-1 block text-foreground sm:mt-2">finally on the page.</span>
          </h1>
          <p className="hero-support-copy order-2 min-w-0 max-w-full text-sm leading-6 text-pretty text-muted-foreground lg:hidden">
            Turn one idea into a complete, editable story. Your first short story is included—no
            card required.
          </p>
          <p className="hero-support-copy hidden min-w-0 max-w-xl text-base leading-7 text-pretty text-muted-foreground lg:mt-8 lg:block sm:text-lg sm:leading-8">
          ...
          <p className="folio-label order-5 min-w-0 text-muted-foreground lg:mt-4">
            Bedtime story / street mystery / family memoir
          </p>
          <div className="order-3 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center lg:mt-9">
            <Link
              href="/studio"
              prefetch={false}
            ...
            <Link
              ...
              className={cn(
                ...
                "hidden min-h-11 rounded-sm px-5 text-base text-muted-foreground lg:inline-flex",
              )}
            >
